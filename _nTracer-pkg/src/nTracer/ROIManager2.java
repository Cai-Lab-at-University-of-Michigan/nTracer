 ...
 */
public class ROIManager2 extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener {
    ...
    private Panel panel;
    private Frame instance; // remove static to make multi -LAW
    ...
    /**
     * Adds the specified ROI to the list. The second argument ('n') will be
     * used to form the first part of the ROI label if it is zero or greater.
     *
     ...
     * to form the first part of the ROI label if it is zero or greater.
     *
     ...
     *
     * @see
     * <a href="http://imagej.nih.gov/ij/macros/js/MultiMeasureDemo.js">JavaScript
     * example</a>
     */
    ...
    /**
     * Intersection of area rois or PointRois. If there is one PointRoi in the
     * list of selected Rois, the points inside all selected area rois are kept.
     * If more than one PointRoi is selected, the PointRois get converted to
     * area rois with each pixel containing at least one point selected.
     */
    void and() {
        ImagePlus imp = getImage();
        if (imp == null) {
     ...
     */
    public ROIManager2 getRoiManager() {
        if (instance != null) {
    ...
    /**
     * Returns a reference to the ROI Manager, or null if it is not open and a
     * batch mode macro is not running. If the ROI Manager is not open and a
     * batch mode macro is running, returns the hidden batch mode RoiManager.
     *
     * @see #getRoiManager
     */
    public ROIManager2 getInstance() {
    ...
    public ROIManager2 getRawInstance() {
        return (ROIManager2) instance;
    }
     ...
     */
    public ROIManager2 getInstance2() {
        ROIManager2 rm = getInstance();
        return rm;
    }
    ...
    public String getName(String index) {
        ...
        ROIManager2 instance = getInstance2();
     ...
     * false if
     * <code>cmd</code> is not "Open", "Save" or "Rename", or if an error
     * occurs.
     */
    ...
    public String getIndexesAsString() {
        ROIManager2 rm = this.getInstance();
        if (rm == null) {
            return "";
        }
    ...
    private class MultiMeasureRunner implements Runnable {
        ...
        public void run() {
            int currentSlice = imp.getCurrentSlice();
            ResultsTable rtMulti = ROIManager2.multiMeasure(imp, rois, appendResults);
            mmResults = (ResultsTable) rtMulti.clone();
            rtMulti.show("Results");
            imp.setSlice(currentSlice);
            if (rois.length > 1) {
